...
All four views use the search options (Aa, ab, .*) and the filters above, but not the main pattern; the term cloud and co-mentions also honour the chosen account, and all hide authors below **min posts per author**. Matching is case-insensitive unless **Aa** is on. Phrases are found automatically: when a two-word combination accounts for most uses of its rarer word ("gas prices", "regime change") it becomes one entry and its uses come out of the single words. URLs and @mentions are dropped, #hashtags and emoji are kept, and English (plus common Spanish, German, French and Portuguese) function words are ignored. Click a word to search the main list for it, together with the term and the author when they apply; click a co-mention row or a heat-map cell for the posts behind it. The author clouds and the heat map never apply the chosen account (they compare authors), so every cloud stays on screen while you read one author's posts below; that author's tile is outlined, and a **← Back** pill (in the panel and above the results) restores the account and pattern from before the click. **Alt-click** (Option-click) a word in any cloud to open its own term cloud. **X search tray.** **Shift-click** words in any cloud (the Topics clouds or the account's cloud in the pane), or accounts anywhere they appear (author rows, @handles and reply/quote tags on cards, circles on the map, rows in the pane), to gather them in a tray, then **release Shift** to open one X search for all of them, on the Latest tab, in a background tab so several can go out in a row and capture pulls the results in. The header's **X search** button opens the same tray empty, to type into: the account box suggests captured accounts and adds any other handle as typed (Enter, space or comma; a pasted list splits), and the word box adds a word or phrase on Enter. Every word chip must appear; **|** inside a chip means either, so `iran | regime` and `overthrow | replace` search `(iran OR regime) (overthrow OR replace)`. Click a word chip to edit it; Shift+Option-click a cloud word to join it to the last chip with |. With accounts in the tray, it picks whose posts: **These N** (`(from:a OR from:b)`; with no words, their combined timeline), **Everyone else** (`-from:a -from:b`, to find other accounts on the same words) or **Everyone**; Shift+1/2/3 switches while Shift is still down, and the choice is remembered. When the words come from an account's cloud (or an account is chosen), that account joins with the first word. Words picked from a cloud narrowed by a term start from the term and its trail. A long account list is split over several searches, each under X's 500-character limit, and the tray says how many will open. Touching the tray, or **Keep open**, holds it open when Shift comes up, so you can gather from anywhere and then press Enter or **Search X ↗**; Esc cancels. **Show as table** gives the numbers behind any view. The panel's **Export** menu takes an optional image title and an image theme (as the page, light or dark) and downloads the image as SVG (1080 × 1080) or PNG (2160 × 2160) with a "what one person scrolled past" caveat printed on it, or the numbers as CSV.
